...
<head>
	...
	<script>
		function valid(){
			 confirm("Valider la commande de produits?");
		}
			...
			<p>Vous avez <?php echo count_products(unser("../../private/passwd")); ?> commandes en attente pour un total de <?php echo count_commands(unser("../../private/passwd"))." produits"; ?></p><br>
				...
				while ($tab[$i])
				{
					if ($tab[$i]['order'])
					{
						...
						show_commands($tab[$i]['order'], $tab[$i]['login']);
						echo ("<ul>");
				...
				<form action="val_destroy_commands.php" method="post">
					...
					<input onclick="return confirm_del()" type="submit" name="delete_order" value="Annuler la commande">
				</form>
			</div>
		</div>
	</div>
